...
def main(video_dir):
    ...
    print(videoname)
    videoWriter = cv2.VideoWriter(videoname+'.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 20, (224,224))
    for idx, frame in enumerate(frames):
        if idx == 0:
            ...
            dets = detector(img, 0)
            print(len(dets))
            bbox = [dets[0].left()+1,dets[0].top()+1,dets[0].right()-dets[0].left(),dets[0].bottom()-dets[0].top()]
